...
def predict(W, b, x):
    result = []
    for i in range(len(W)):
        result.append(calc_sigmoid(calc_z(W[i], x, b[i])))
...
W = [init_weights(784) for i in range(10)]
b = [0 for i in range(10)]
X = [training_img[i] for i in range(trainig_sets)]
Y = [[normalize_val(training_vals[i], j) for i in range(trainig_sets)] for j in range(10)]
...
for i in range(400, 500):
    result, prob = predict(W, b, training_img[i])
